...
Function CreateApp {
    ...
    ## MgGraph
    New-MgApplication -DisplayName "$ApplicationName"
}
Function GetApp {
    param (
        [Parameter(Mandatory=$true, Position=0)]
    ...
    ## MgGraph
    If (Get-MgApplication -Filter "DisplayName eq '$ApplicationName'"){
        $Count = (Get-MgApplication -Filter "DisplayName eq '$ApplicationName'").Count
        If ($Number -gt 1){
            Write-Host "!!! You have more than 1 application" -ForegroundColor Red
            Exit
        }
...
Function GetAppId {
    ...
    ## MgGraph
    $AppObjectIdValue = (Get-MgApplication -Filter "DisplayName eq '$ApplicationName'").Id
...
Function AddKey {
    ...
    $params = @{
        KeyCredential = @{
            ...
            usage = "Verify"
            key = $KeyValue
            endDateTime = $($Certificate.GetExpirationDateString())
    ...
    Add-MgApplicationKey -ApplicationId $ObjectId -BodyParameter $params
}
